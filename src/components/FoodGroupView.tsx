...
const FoodGroupView: React.FC<Props> = (props) => {
    ...
    const [loadingFoodGroup, isLoadingFoodGroup, error] = useIsLoading(async () => {
        ...
        const response = props.source == RequestSource.FoodGroup ?
            await GetService.GetFoodGroup(props.idFoodGroup)
            : await GetService.GetSanPinGroup(props.idFoodGroup);

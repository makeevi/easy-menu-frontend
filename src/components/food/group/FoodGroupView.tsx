...
import FoodGroupItem, { RequestSource } from '../components/ListFoodInGroupComponent';
...
import FoodGroupTabTemplate from '../components/FoodGroupTabTemplate';
...
<table className="table">
  <thead>
    <tr>
      <th scope="col" ><LinkPrimaryButton role={Role.add} onClick={ShowModal} /></th>
...
{lodash.map(foodGroups?.sort((a, b) => a.StartDate < b.StartDate ? 1 : -1), function (item, index){
  console.log(item);
  return <FoodGroupTabTemplate key={item.Id} id={item.Id} index={index} name={item.Name} count={item.Count} start_date={item.StartDate} delete={DeleteFoodGroup}/>
})}
